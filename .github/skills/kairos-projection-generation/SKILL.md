...
- **dbt**: When the ontology drives a data warehouse using dbt Core. Generates a complete dbt project with staging models (from bronze source systems via SKOS mappings), silver entity models, schema YAML with SHACL-derived tests, and project config. Requires `output/medallion/bronze/*.ttl` for source system descriptions and `integration/mappings/*.ttl` for SKOS column mappings. See the **kairos-medallion-projection** skill.
