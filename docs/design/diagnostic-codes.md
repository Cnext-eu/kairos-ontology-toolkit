...
| `dbt-source.dependency-unresolved` | error | DD-133 (default) | A selected model's transitive `ref()` target is missing or ambiguous under `integration/transforms/dbt/models/` and (since #586) under `integration/transforms/dbt/seeds/` — a `ref()` may resolve to exactly one authored seed CSV leaf instead of a model; projection cannot emit a self-contained project otherwise. |
