...
> Companion to [DD-133](decisions/dd-133-v5-authoring-break--yaml-entitybinding--stateless-compile-companion.md). This is the stable catalog
...
[`docs/dev/cli-behaviour-notes.md`](cli-behaviour-notes.md#validate-dbt-contracts-vs-validate-dbt).
...
| `relationship.external-reference-key-column-unknown` | error | DD-133-safety | #775. An `externalReference.key[].column` naming a column the parent domain's declared Silver contract (DD-213) does not materialize. The key names the *parent's* output column, but the rename happens in the binding, so the source-side name is the one an author has just been looking at -- four bindings in one hub drifted this way independently and all four passed CI, because `--emit` rendered the join and only `dbt run` against a real warehouse rejected it. Checkable in per-domain scope without weakening DD-133/140: `discover_contract_paths` already admits a foreign contract whenever this domain points a relationship at a class it declares, and already hashes it into provenance -- `_foreign_contract_columns` merely parses what was already an input, and consults the parent's *declared* interface, never its emitted artifacts. Keyed on the resolved target class URI, not `externalReference.name`, so the verdict does not depend on the parent's model-naming rule. Fails open by construction: an ungoverned parent, an unloadable contract, or a class that will not resolve leaves the reference unchecked (DD-213 §6 -- a domain without a contract compiles exactly as it did before), so this can only ever reject a reference a contract positively contradicts. Distinct from `safety.relationship-endpoint`, which the neighbouring `join.foreign` vs `key` check uses: eleven sites construct that code and only a distinct literal is pinnable by a test. |
| `relationship.external-reference-key-unverified` | info | DD-133-safety | #934. The note that `relationship.external-reference-key-column-unknown` could not run: no Silver contract in scope declares the parent class, so the key was not checked. Most hubs author no contracts, so that check used to be skipped for every relationship without a word, and a join naming a column the parent never emits passed compile, emit and `audit-silver-samples` alike. Info, never blocking: an info-level relationship diagnostic does not filter the binding from the IR. The remedy names both routes -- declare the parent contract (DD-213), or run `validate`, whose hub-wide `relationship.external-reference-key-not-emitted` warning checks the key against the parent's binding. |
| `binding.target-class-deprecated` | warning | DD-133-safety | #938. The binding's `target.class`, or one of its ancestors, is marked `owl:deprecated true` in the import closure. The reference models deprecate the role subclasses (Consignee, Carrier, NotifyParty, ...) that `blueprints/patterns/qualified-role-assignment` forbids a hub to build on, and the class comment names the replacement, which the message quotes. Computed once per scope from the graph into `ResolutionContext.deprecated_classes`; the adapter stays graph-free. Warning: the binding is correct and emittable, the shape is what is wrong. |
| `binding.carried-outnumbers-canonical` | warning | DD-139 | #854. A binding with strictly more `technicalFields` of `purpose: carried` than ontology-backed `fields:`. Carried columns reach Silver as raw source values with no ontology property behind them; legitimate one at a time, but a model mostly made of them is semantically hollow while looking canonical downstream (one hub: 89 carried against 82 backed). Per binding, never a hub average, which would hide the model that needs attention. Warning: the model is correct and emittable, and a genuinely raw feed may stay above the line. |
| `relationship.optional-but-ontology-requires` | warning | DD-241 | #999. The ontology requires the relationship (an `owl:minCardinality`/exact restriction ≥ 1 on the bound class or an ancestor), but the binding says `missingParent: null`, so Silver loads rows with no parent. The Silver ERD then draws the parent end optional (`|o`) where the class and contract diagrams draw it mandatory. Warning: the binding may be right and the ontology stale. Bounds come from `erd_projector.edge_multiplicities`, precomputed per (class, property) in `resolve_scope` into `ResolutionContext.relationship_bounds`. |
| `relationship.one-to-one-not-in-ontology` | warning | DD-241 | #999. The binding declares `cardinality: one-to-one`, but OWL does not bound the inverse to one (no `owl:InverseFunctionalProperty`, no functional or max-1 `owl:inverseOf` partner). The Silver ERD draws the child end `o|` from the binding; the ontology says a parent may have many children. Warning, same reasoning. |
| `relationship.unrealized-technical-field` | warning | DD-139 | #491. A binding with one or more `technicalFields` entries of `purpose: relationship` but an empty `relationships:` list. The carrier is the documented way to keep a foreign-key column in Silver when the join cannot be authored yet (an unbound cross-domain parent, or the self-reference shape below), so authoring one is legal — but never following up leaves the FK in Silver as a raw column with no join, no surrogate key and no orphan window. Warning, not error: the binding is correct and emittable, and staging carriers ahead of the parent domain is legitimate; warnings never fail `CompileResult.succeeded`. Blind spot by construction — it cannot fire when the FK column was authored under a different `purpose` (the CLdN `Qlik-routes` binding marks every carrier `purpose: identity`); `kairos-ontology propose-relationships` covers that case by matching join keys against other bindings directly. The remedy text names what that command cannot do, because pointing at it unconditionally sent authors after a tool with nothing to offer: measured on one hub, five of nine warnings named bindings appearing as a child in no proposal at all. The unresolvable causes are structural — no object property between the two bound classes, a polymorphic carrier, or the container-to-contained shape where the key is on the parent (#723). |
...
| `safety.column-unresolved` | error | DD-133 (default) | Remap of `binding.unknown-column` / `binding.unknown-key-column`. Fires for a column that is genuinely absent *and* for one whose declared physical type has no entry in `_SOURCE_TYPE_ALIASES` — such a column never reaches the bound relation's symbol table, so it is indistinguishable from a missing column at the point of use. The message distinguishes them: when the type is the cause it names the type and points at `suggest-type`, because the bare "not a column of the bound relation" sent four separate authors on one hub hunting a schema problem that did not exist (#808, same reasoning as #751). |
