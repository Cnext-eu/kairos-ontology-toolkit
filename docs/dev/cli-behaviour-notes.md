...
declared-contract ERDs, bound Silver and Gold ERDs and their masters — opens with four lines of
frontmatter selecting the ELK layout engine, followed by the `%% Generated by kairos-ontology`
stamp, then the diagram keyword. ELK routes edges orthogonally and clusters the stub nodes every
ERD family draws outside its main group; dagre, Mermaid's default, scatters them and draws long
diagonals across the canvas. Layout happens at render time, so the rest of the file is
byte-identical and adopting this is one deterministic diff across a hub's tracked diagrams.

Where a renderer has no ELK registered (GitHub, Azure DevOps wikis) Mermaid logs a warning and
falls back to dagre — today's picture, no error. `mmdc` and the VS Code Mermaid Chart extension
both register ELK. A renderer older than Mermaid 10.5 rejects frontmatter outright; a hub on such
a surface sets `projections.mermaid_layout: none` in `kairos.yaml` and gets the pre-#855 bytes.
The setting is hub-wide on purpose: a per-target switch would let one hub's diagrams disagree
about their own layout. `core/projections/shared.py::mermaid_header` is the single helper, so the
frontmatter-before-comment order cannot drift between projectors, and the master-ERD merges strip
each part's header with `strip_mermaid_header` so the merged file carries exactly one.
