...
- **Why:** Power BI refuses to load a model with two active paths, so the projector keeps a spanning forest and deactivates the rest (#792). A deactivated edge filters nothing unless a measure calls USERELATIONSHIP: on the Fracht hub `fact_consignment -> dim_job` was inactive, so "consignments by job type" showed the grand total on every row, and nothing reported it. Declare the edge that must stay active with kairos-ext:goldPrimaryRelationship, remove the redundant route, or record why the deactivation is intended.
