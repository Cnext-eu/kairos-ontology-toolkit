...
description: >
  ...
  use kairos-ontology-projection for that.
---
...
2. **Coverage threshold gate**: if domain property coverage < 50%, warn:
   ...
   > ontology needs additional properties (invoke kairos-ontology-modeling) or
...
3. Offer next actions:
   ...
   - Proceed to projection (`kairos-ontology-projection`)
...
| Skill | Relationship |
|---|---|
| `kairos-ontology-medallion-source` | **Upstream** — creates bronze vocabulary (input to this skill) |
| `kairos-ontology-modeling` | **Upstream** — creates domain ontology (target for mappings) |
| **`kairos-ontology-mapping`** (this) | Creates SKOS mapping files interactively |
| `kairos-ontology-mapping-report` | **Downstream** — generates HTML coverage reports from mappings |
| `kairos-ontology-medallion-silver` | **Downstream** — uses mappings for extension annotations |
| `kairos-ontology-projection` | **Downstream** — generates dbt models from mappings |

### Typical pipeline order

```
1. kairos-ontology-modeling        → domain ontology (.ttl)
2. kairos-ontology-medallion-source → bronze vocabulary (.vocabulary.ttl)
3. kairos-ontology-mapping          → SKOS mapping files (model/mappings/)
4. kairos-ontology-medallion-silver → silver extension annotations
5. kairos-ontology-projection       → dbt/silver/powerbi output
```
