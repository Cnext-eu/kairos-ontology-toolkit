...
| Coverage report | `output/reports/coverage-dbt-*.json` (most recent) |
...
| What to check | Where | What it tells you |
|---------------|-------|-------------------|
| Domain coverage (silver NULL columns) | `output/reports/coverage-dbt-*.json` (most recent) | Per-entity: total properties, populated from source, always-NULL columns, missing required mappings |
| Industry alignment (ontology vs ref model) | `output/reports/coverage-*.yaml` or `output/reports/coverage-*.md` (most recent) | Per-domain: class/property alignment % against reference models, custom vs aligned fields. ⚠️ Requires LLM — generated by `kairos-ontology coverage-report` (needs `GITHUB_TOKEN` or Azure AI credentials). |
| Source mapping reports | `output/reports/*-mapping-report-*.html` | Per-source-system: mapped vs unmapped columns, match types, action items |

**Steps:**
1. Check if `output/reports/coverage-dbt-*.json` exists. If so, use the most recent:
...
2. Check if `output/reports/coverage-*.yaml` or `output/reports/coverage-*.md` exists
   (excluding `coverage-dbt-*`). If so, use the most recent:
...
3. Check if `output/reports/` contains `*-mapping-report-*.html` files. Count them.
...
ls -t output/reports/coverage-dbt-*.json 2>/dev/null | head -1
ls -t output/reports/coverage-*.yaml output/reports/coverage-*.md 2>/dev/null | head -1
ls output/reports/*-mapping-report-*.html 2>/dev/null | wc -l

# Read dbt coverage summary (most recent file)
python -c "
import json, pathlib
files = sorted(pathlib.Path('output/reports').glob('coverage-dbt-*.json'))
if files:
...
- **Path:** `output/reports/status-{YYYY-MM-DD-HHmmss}.md` (relative to the hub root).
  Use the current UTC timestamp for `{YYYY-MM-DD-HHmmss}` (e.g. `status-2026-06-10-205500.md`).
...
**Steps:**
1. After assembling the full report, write it to `output/reports/status-{ts}.md` using
   the `create` or `edit` tool (or `echo` / `Set-Content` if the tool isn't available).
2. Tell the user: "📄 Report saved to `output/reports/status-{ts}.md`."
