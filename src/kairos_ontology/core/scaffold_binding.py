...
from ._samples import is_pii_column, is_redaction_token
...
# --------------------------------------------------------------------------------------
def partition_pii_columns(
    columns: tuple[SourceColumn, ...], *, include_pii: bool = False
) -> tuple[tuple[SourceColumn, ...], tuple[SourceColumn, ...]]:
    """Split *columns* into ``(kept, excluded)`` under the DD-075 privacy posture (#758).

    A column is excluded when :func:`kairos_ontology.core._samples.is_pii_column` classifies
    its name or its sampled values as personal data, or when the import redactor already
    replaced one of its samples with an opaque ``<redacted ...>`` token -- that token is the
    import step's own verdict that the column holds PII. With *include_pii* nothing is
    excluded; the second tuple then still names the columns that *would* have been, so a
    caller can say what it kept.
    """
    flagged = tuple(
        column
        for column in columns
        if is_pii_column(column.name, sample_values=list(column.samples))
        or any(is_redaction_token(sample) for sample in column.samples)
    )
    if include_pii:
        return columns, flagged
    flagged_names = {column.name for column in flagged}
    kept = tuple(column for column in columns if column.name not in flagged_names)
    return kept, flagged


def _privacy_header_lines(
    flagged: tuple[SourceColumn, ...], *, include_pii: bool
) -> list[str]:
    """Comment lines naming the personal-data columns a staging model dropped (or kept).

    Must never contain the substring ``with`` followed by whitespace: dbt-fabric counts
    that substring across the whole file, comments included, to detect a nested CTE
    (``dbt-contract.dialect-fabric-nested-cte``), and the model body already spends the one
    occurrence it is allowed on its CTE opener.
    """
    if not flagged:
        return []
    names = ", ".join(column.name for column in flagged)
    if include_pii:
        lead = (
            f"-- privacy: --include-pii was passed; {len(flagged)} personal-data column(s) "
            "kept: "
        )
    else:
        lead = "-- excluded by privacy policy (pass --include-pii to keep): "
...
def render_staging_sql(
    ...
    *,
    ...
    Columns that :func:`partition_pii_columns` classifies as personal data are left out of
    the SELECT and named in the header comment instead (#758); *include_pii* restores them.
    """
    from .projections.medallion_dbt_projector import _source_type_to_target

    kept, flagged = partition_pii_columns(columns, include_pii=include_pii)
    lines = [
        ...
        "-- productionizing.",
        *_privacy_header_lines(flagged, include_pii=include_pii),
        "",
        "with source as (",
...
def run_scaffold_binding(
    ...
    *,
    ...
    The passthrough staging model leaves out columns :func:`partition_pii_columns` classifies
    as personal data (credentials, payroll, contact and demographic fields) and names them in
    its header comment and in ``notes`` instead; ``include_pii=True`` keeps them (#758).
    ...
    if archetype.tier == "passthrough":
        _kept, pii_columns = partition_pii_columns(columns, include_pii=include_pii)
        if pii_columns:
            pii_names = ", ".join(column.name for column in pii_columns)
            if include_pii:
                notes.append(
                    f"privacy: --include-pii kept {len(pii_columns)} personal-data column(s) "
                    f"in the staging model: {pii_names}."
                )
            else:
                notes.append(
                    f"privacy: {len(pii_columns)} personal-data column(s) excluded from the "
                    f"staging model (pass --include-pii to keep them): {pii_names}."
                )
        dbt_model_text = render_staging_sql(
            system, table, columns, platform=platform, include_pii=include_pii
        )
        dbt_model_path = (
            hub_root
            / "integration"
