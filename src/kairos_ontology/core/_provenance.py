...
def read_provenance_version(ttl: str) -> str | None:
    """Return the toolkit version stamped on *ttl*'s provenance header, if any.

    Reads only the ``# Generated by kairos-ontology-toolkit vX.Y.Z`` comment
    line already written by ``provenance_comment``/``prepend_provenance`` — it
    does not invent or infer a version when no header is present (returns
    ``None``). Used to report the *prior artifact generator version* alongside
    the *running* toolkit version without fabricating provenance.
    """
    lines = ttl.splitlines()
    if not lines or lines[0].strip() != _RULE:
        return None
    prefix = f"# Generated by {_MARKER} v"
    for line in lines[1:]:
        stripped = line.strip()
        if stripped == _RULE:
            break
...
def running_toolkit_version() -> str:
    """Return the currently running toolkit version (public wrapper)."""
