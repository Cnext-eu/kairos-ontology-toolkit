...
class AzureSearchProjector:
    ...
    def __init__(
        self, ontology_path: Path, output_dir: Path, template_dir: Path, mappings_dir: Path = None
    ):
            ...
            fields.append(
                {
                    ...
                    "facetable": edm_type
                    in ["Edm.String", "Edm.Int32", "Edm.Int64", "Edm.Boolean"],
                }
            )

        return fields

    def generate_index_definition(
        self, class_info: Dict, fields: List[Dict], synonym_maps: List[str]
    ) -> str:
    ...
    def generate_synonym_map(self, class_info: Dict, synonyms: List[str]) -> str:
        ...
        solr_line = self.skos_parser.generate_solr_synonyms(
            class_info["name"], class_info["label"], synonyms
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("ontology-hub-publish/azure-search"),
        help="Output directory",
    )
    parser.add_argument(
        "--templates",
        type=Path,
        default=Path("scripts/templates/azure-search"),
...
def generate_azure_search_artifacts(
    classes: list,
    graph,
    template_dir,
    namespace: str,
