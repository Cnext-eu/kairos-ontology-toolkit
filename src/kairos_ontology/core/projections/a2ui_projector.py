...
class A2UIProjector:
    ...
    def __init__(
        self, ontology_path: Path, output_dir: Path, template_dir: Path, shapes_dir: Path = None
    ):
            ...
            classes.append(
                {
                    ...
                    if row.comment
                    else f"{class_name} message payload",
                }
            )
    ...
    def generate_message_schema(
        self, class_info: Dict, properties: List[Dict], required: List[str]
    ) -> str:
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("ontology-hub-publish/a2ui"), help="Output directory"
    )
    parser.add_argument(
        "--templates", type=Path, default=Path("scripts/templates/a2ui"), help="Template directory"
    )
