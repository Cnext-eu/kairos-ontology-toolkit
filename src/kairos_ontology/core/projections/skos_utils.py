...
class SKOSParser:
    ...
    def get_synonyms_for_class(self, class_name: str, graph: Graph = None) -> List[str]:
        ...
        synonyms = []
        for alt_label in graph.objects(
            subject=self.KAIROS[f"{class_name}Concept"], predicate=self.SKOS.altLabel
        ):
    ...
    def generate_solr_synonyms(
        self, class_name: str, preferred_label: str, synonyms: List[str]
    ) -> str:
        """
