...
#: Hub-wide root artifact: fabric-cicd reads exactly one per `repository_directory`,
#: so every domain's Gold emit writes this same shared path. Public because
#: `cli/emit_gold.py` must declare it mergeable by the identical string (issue #664).
PARAMETER_ARTIFACT_PATH = "parameter.yml"
...
def gold_dbt_artifact_paths(spec, physical) -> set[str]:
    """Return every dbt path :func:`render_gold_dbt_artifacts` will produce.

    The compiler intersects rendered artifacts with ``_planned_artifact_paths``, so a
    Gold model that renders but is not planned is silently dropped before anything is
    written -- which is exactly why ``compile --emit`` never wrote the Gold dbt models
    the medallion ``dbt_project.yml`` was already configured to host (issue #665).
    Deriving both the planned set and the render-completeness check from this one
    function is what stops the two from drifting apart again.
    """
    paths = {physical.dbt_schema_artifact_path, physical.exposures_artifact_path}
    for table in physical.tables:
        paths.add(f"models/gold/{spec.ontology_name}/{table.name}.sql")
        if table.dual_current_name:
            paths.add(f"models/gold/{spec.ontology_name}/{table.dual_current_name}.sql")
    if spec.calendar is not None and spec.calendar.approved:
        paths.update(
            {
...
def render_powerbi_artifacts(
    ...
    *,
    ...
    # The Gold dbt models are NOT re-emitted here. `powerbi/<domain>/dbt/` has no
    # `dbt_project.yml`, so dbt's `packages.yml` `subdirectory:` mechanism cannot install
    # it -- those copies could only ever be hand-copied downstream, and went stale
    # silently on the next emit (issue #665). They ship in the medallion package
    # instead, via `compile --emit`, refreshed by the same `bump-hub` cycle as Silver.
