...
from .prep_renderers import (
    render_prep_array_child,
    render_prep_model,
    render_prep_schema,
)
...
from .specs import CoverageSpec, SchemaKind, SilverModelKind, SourceCatalogSpec

logger = logging.getLogger(__name__)


class PreparationRenderBlocked(ValueError):
    """Required prep behavior has no valid physical implementation."""
    ...
    def __init__(self, blocking_rules: tuple[tuple[str, str], ...]) -> None:
        ...
        super().__init__(
            "Preparation materialization blocked by adapter capabilities: "
            f"{detail}"
        )
...
def _mapping_expression_data(expression: MappingExpression) -> dict[str, object]:
    ...
    result: dict[str, object] = {
        ...
        "capability_requirements": [
            item.value for item in metadata.capability_requirements
        ],
    ...
    if isinstance(expression, SourceColumnExpression):
        ...
        result["arguments"] = [
            _mapping_expression_data(item) for item in expression.arguments
        ]
...
def _render_source(spec: SourceCatalogSpec, env: Environment) -> str:
    return env.get_template("sources.yml.jinja2").render(
        ...
        schema=spec.schema,
        tables=[
            {"name": table.name, "label": table.label} for table in spec.tables
        ],
...
def render_project(
    ...
    prep_physical = {item.model_name: item for item in plan.prep_models}
    ...
    prep_blockers = tuple(
        reason
        for prep_plan in plan.prep_models
        for reason in prep_plan.blocking_reasons
    )
    if prep_blockers:
        raise PreparationRenderBlocked(prep_blockers)
    ...
    if plan.policy is not None:
        artifacts["contracts/dq-runtime-result-contract.schema.json"] = (
            render_dq_runtime_contract(plan.policy.dq_runtime_result)
        )
    for model in shaped.prep_models:
        artifacts[model.artifact_path] = render_prep_model(
            model,
            prep_physical[model.model_name],
            plan.adapter.platform,
        )
    for child in shaped.prep_children:
        artifacts[child.artifact_path] = render_prep_array_child(
            child,
            prep_physical[child.model_name],
            plan.adapter.platform,
        )
    for document in plan.prep_documents:
        artifacts[document.artifact_path] = render_prep_schema(
            shaped.prep_models,
            shaped.prep_children,
            plan.prep_models,
            document,
        )

    if plan.release.unbound_eligible_names:
        artifacts["__unbound_eligible__"] = list(
            plan.release.unbound_eligible_names
        )
    ...
    for document in shaped.schema_documents:
        ...
        artifacts[document.artifact_path] = render_schema_models(
            ...
            physical_models=(
                plan.silver.models if plan.silver is not None else ()
            ),
        )
    ...
    if shaped.gold_product is not None and plan.gold is not None:
        artifacts[
            f"metadata/{shaped.gold_product.ontology_name}-gold-product.json"
        ] = json.dumps(
            gold_product_report(
                shaped.gold_product,
                plan.gold,
    ...
    artifacts["__release_data__"] = {
        ...
        "binding_status": {
            ...
            "unbound_eligible": list(plan.release.unbound_eligible_names),
        },
        "coverage_status": {
            "status": (
                "blocking"
                if shaped.coverage is not None
                and any(
                    entity.missing_required_mappings
                    for entity in shaped.coverage.entities
                )
                else "ready"
            ),
        ...
        "adapter_compile_evidence": [
            {
                ...
                "capabilities": [
                    capability.value
                    for capability in evidence.capabilities.value
                ],
        ...
        "deviations": [
            {
                ...
                "adapter": (
                    deviation.adapter.value
                    if deviation.adapter is not None
                    else None
                ),
        ...
        "dq_runtime_result_contract": {
            ...
            "statuses": [
                status.value for status in shaped.policy.dq_runtime_result.statuses
            ],
            "fields": [
                {
                    "name": field.name,
        ...
        "gold_status": {
            ...
            "security": (
                ...
                or shaped.policy.gold.security is None
                else "ready"
            ),
            "calendar": (
                "not-applicable"
                if shaped.policy.gold.profile is None
                or shaped.policy.gold.calendar is None
                else "ready"
        ...
        "identity_lineage": [
            {
                ...
                "natural_key_columns": list(
                    authority.entity_identity.business.keys.value
                ),
                "integration_key_emitted": (
                    authority.entity_identity.integration.emitted
                ),
                "mastered_identifier_refs": list(
                    authority.entity_identity.mastered.external_identifier_refs.value
                ),
                "entity_instance_iri_policy": (
                    authority.entity_identity.iri.mode.value.value
                ),
                "driving_source": {
                ...
                "roles": [
                    {
                        ...
                        "establishes_business_identity": (
                            role.establishes_business_identity
                        ),
                ...
                "multi_source": (
                    {
                        ...
                        "normalization": (
                            authority.multi_source.normalization.statement.value
                        ),
                        "precedence_mode": (
                            authority.multi_source.precedence.mode.value.value
                        ),
                ...
                "contribution_lineage": (
                    {
                        ...
                        "parent_key_column": (
                            authority.contribution_lineage.parent_key_column
                        ),
                        ...
                        "source_role_column": (
                            authority.contribution_lineage.source_role_column
                        ),
                ...
                "reconciliation_limitation": (
                    ...
                    if authority.entity_identity.surrogate.reconciliation_limitation
                    is not None
                    else None
                ),
            }
        ...
        "runtime_semantics": [
            {
                ...
                "merge_identity": list(
                    runtime.authority.incremental.merge_identity.value
                ),
                ...
                "ingested_at": (
                    runtime.authority.incremental.ordering.ingested_at.value
                ),
                ...
                "soft_delete": (
                    runtime.authority.incremental.soft_delete.value.value
                ),
                "delete_semantics": {
                ...
                "late_arrival": (
                    runtime.authority.incremental.late_arrival.value.value
                ),
                "correction": (
                    runtime.authority.incremental.correction.value.value
                ),
                ...
                "evidence": {
                    ...
                    "ordering": [
                        (
                            runtime.authority.incremental.ordering.tie_breakers.provenance
                            .predicate_uri
                        ),
        ...
        "temporal_foreign_keys": [
            {
                ...
                "interval": (
                    ...
                    if relationship.interval is not None
                    else None
                ),
                "time_zone": (
                    relationship.time_zone.value
                    if relationship.time_zone is not None
                    else None
                ),
                "precision": (
                    relationship.precision.value
                    if relationship.precision is not None
                    else None
                ),
        ...
        "preparation_routes": [
            {
                "relation_uri": route.relation_uri,
                "source_name": route.source_name,
                "table_name": route.table_name,
                "mode": route.mode,
                "ref_model": route.ref_model,
                "rule_id": route.rule_id,
            }
            for route in plan.release.prep_routes
        ],
        "blocking_reasons": [
    ...
    expected_paths.update(model.artifact_path for model in shaped.prep_models)
    expected_paths.update(child.artifact_path for child in shaped.prep_children)
    expected_paths.update(document.artifact_path for document in plan.prep_documents)
    ...
    for quality in plan.quality_models:
        ...
        for rule in quality.rules:
            quality_paths.update(
                {rule.result_artifact_path, rule.test_artifact_path}
            )
    ...
    if shaped.gold_product is not None and plan.gold is not None:
        ...
        for table in plan.gold.tables:
            expected_paths.add(
                f"models/gold/{shaped.gold_product.ontology_name}/{table.name}.sql"
            )
            if table.dual_current_name:
                expected_paths.add(
                    "models/gold/"
                    f"{shaped.gold_product.ontology_name}/"
                    f"{table.dual_current_name}.sql"
                )
        if (
            shaped.gold_product.calendar is not None
            and shaped.gold_product.calendar.approved
        ):
            expected_paths.update(
                {
    ...
    missing_artifacts = sorted(
        path for path in expected_paths if path not in artifacts
    )
    ...
    release_data["review_disposition"] = (
        "blocking"
        if release_data["blocking_reasons"] or missing_artifacts
        else "supported"
    )
    artifacts[
        f"metadata/{plan.release.ontology_name}-release-review.json"
