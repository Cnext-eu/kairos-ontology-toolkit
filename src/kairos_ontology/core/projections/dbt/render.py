...
Gold products) emits into this one `output/medallion/dbt` project. Point `dbt` and
`kairos-ontology validate-dbt` at this directory — there is no per-domain dbt project.
    ...
    artifacts["__release_data__"] = {
        ...
        "gold_status": {
            ...
            "calendar": (
                ...
                else "ready" if shaped.policy.gold.calendar.approved else "blocking"
            ),
            "measures": (
                "not-applicable"
            ...
            "tables": (
                "not-applicable"
                if shaped.policy.gold.profile is None
                else "ready" if shaped.gold_product is not None else "blocking"
            ),
            "adapter": (
                "not-applicable"
                if shaped.policy.gold.profile is None
                else "ready" if plan.gold is not None else "blocking"
            ),
            "tmdl_compile": (
                "not-applicable"
                if shaped.policy.gold.profile is None
