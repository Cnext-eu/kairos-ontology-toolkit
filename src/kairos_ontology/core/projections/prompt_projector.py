...
class PromptProjector:
    ...
    def generate_context(
        self, template_name: str, concepts: List[Dict], relationships: List[Dict]
    ) -> str:
...
def main() -> None:
    ...
    parser.add_argument(
        "--mappings",
        type=Path,
        default=Path("ontology-hub/model/mappings"),
        help="Path to SKOS mappings directory",
    )
    parser.add_argument(
        "--output", type=Path, default=Path("ontology-hub-publish/prompt"), help="Output directory"
    )
    parser.add_argument(
        "--templates",
        type=Path,
        default=Path("scripts/templates/prompt"),
    ...
    projector = PromptProjector(
        args.ontology, args.output, args.templates, args.mappings, template_types
    )
...
def generate_prompt_artifacts(
    ...
    if semantic_index is not None:
        ...
        def get_index_properties(class_uri: str) -> list:
            ...
            for link in record.direct_properties + record.inherited_properties:
                ...
                range_name = extract_local_name(prop.ranges[0].uri) if prop.ranges else "string"
                simple_type = {
                    "string": "text",
                    "integer": "number",
                    "int": "number",
    ...
    for concept in concepts:
        entity = {"description": concept["description"] or concept["label"]}

        # Add properties in compact format
        if concept["properties"]:
            entity["fields"] = {
                prop["name"]: {"type": prop["type"], "desc": prop["description"]}
                if prop["description"]
                else prop["type"]
    ...
    if relationships:
        llm_context["relationships"] = [
            {"name": rel["name"], "from": rel["from"], "to": rel["to"], "desc": rel["description"]}
            if rel["description"]
            else {"name": rel["name"], "from": rel["from"], "to": rel["to"]}
