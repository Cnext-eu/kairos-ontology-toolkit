...
def generate_gold_from_compile_plans(
    ...
    return render_gold_product(logical, compile_plans, product, physical=physical)
...
def render_gold_product(
    ...
    product: GoldProductConfig,
    *,
    physical: GoldProductPhysicalSpec | None = None,
) -> dict[str, str]:
    ...
    not shape the same product twice. Pass *physical* too when the caller already has it,
    so materialization is not repeated either.
    """
    if physical is None:
        materialized = compile_plans[0].materialization_plan
        physical = materialize_gold_product(
    ...
    artifacts.update(_bus_matrix(logical, product))
    return artifacts


def _bus_matrix(logical: GoldProductLogicalSpec, product: GoldProductConfig) -> dict[str, str]:
    """Render the product's Kimball bus matrix (DD-240): which facts share which dimensions.

    The review artifact for the model-shape practices -- one glance shows a fact with no
    dimensions, a dimension shared by nothing, or a role-playing dimension with no active
    role. Absent for a product with no fact or no dimension.
    """
    from .dbt.gold_shape import CALENDAR_TABLE
    from .dbt.gold_shape_checks import bus_matrix

    calendar = getattr(logical, "calendar", None)
    matrix = bus_matrix(
        tuple(getattr(logical, "tables", ()) or ()),
        tuple(getattr(logical, "relationships", ()) or ()),
    ...
    text = "\n".join(
        [
            f"# {product.display_name or product.name} — bus matrix",
            "",
            "<!-- Generated by kairos-ontology emit-gold (DD-240). Do not edit by hand. -->",
            "",
            "One row per fact, one column per dimension. The model-shape practices it helps "
            "review are in `docs/toolkit/practices/semantic-model.md`.",
            "",
            matrix,
            "",
        ]
    )
    return {f"{product.name}/{product.name}-bus-matrix.md": text}
