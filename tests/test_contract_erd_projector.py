...
class TestContractErd:
    def test_renders_an_er_diagram_per_declared_entity(self, contracts_dir):
        ...
        # Layout frontmatter first (#855), then the toolkit stamp, then the comments.
        assert content.startswith("---\nconfig:\n  layout: elk\n---\n%% Generated by kairos-ontology ")
        assert "%% Declared Silver contract ERD: party" in content
...
class TestTargetRegistration:
    ...
    def test_target_writes_into_the_hub_contract_diagrams_directory(self, tmp_path):
        ...
        # Hub-relative, not publish-relative: the diagram is drawn for human review and
        # the publish root is gitignored in full, so it lands beside its contract.
        written = hub / "model" / "contracts" / "diagrams" / "party-contract-erd.mmd"
        assert written.is_file()
        assert "erDiagram" in written.read_text(encoding="utf-8")
        assert b"\r" not in written.read_bytes()
        assert not (output / "architecture" / "contract-erd").exists()
