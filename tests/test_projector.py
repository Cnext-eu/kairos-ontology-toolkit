...
class TestProjector:
    ...
    def test_load_single_ontology_file_path(self, temp_dir, ontology_files, capsys):
        ...
        assert not (output_dir / "projection-report.json").exists()
    ...
    def test_empty_directory(self, temp_dir, capsys):
        ...
        run_projections(
            ontologies_path=empty_dir, catalog_path=None, output_path=output_dir, target="neo4j"
        )
    ...
    def test_auto_detect_prefers_custom_namespace_over_fibo(
        self, temp_dir, ontology_with_fibo_imports
    ):
    ...
    def test_a2ui_projection_creates_files(self, temp_dir, sample_ontology):
        ...
        run_projections(
            ontologies_path=ontologies_dir, catalog_path=None, output_path=output_dir, target="a2ui"
        )
...
class TestDD021ImportWhitelisting:
    ...
    def _build_graph(self, *, with_silver_include=False, with_bulk=False, with_peer_import=False):
    ...
    def test_bulk_excludes_peer_hub_domains(self):
        ...
        rows = self._all_class_rows(g)
        hub_ns = {self.HUB_NS, self.PEER_NS, self.HUB_NS.rstrip("#"), self.PEER_NS.rstrip("#")}
        result = _discover_whitelisted_imports(
            g,
            self.HUB_NS,
            rows,
