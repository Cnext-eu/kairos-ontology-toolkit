...
from kairos_ontology.core.hub_utils import contract_diagrams_dir, publish_root
...
def test_master_gold_erd_is_written_alongside_the_domain_erd(tmp_path, monkeypatch):
    ...
    the hub's diagrams directory, so it accumulates across separate single-domain
    invocations without this command needing to know about other domains itself.

    Both the per-domain and the merged Gold ERD now land in the *authored* hub, beside
    the Silver and declared-contract diagrams, and no ERD is left in the Power BI
    package -- that whole tree is gitignored, so a diagram written there is a diagram
    nobody reviews.
    """
    hub = _copy_hub(tmp_path)
    monkeypatch.chdir(hub)
    ...
    master_erd = diagrams / "master-gold-erd.mmd"
    assert master_erd.is_file()
    content = master_erd.read_text(encoding="utf-8")
    assert "erDiagram" in content
    assert "party" in content
    assert (diagrams / "party-gold-erd.mmd").is_file()
    assert not list((publish_root(hub) / "powerbi").rglob("*-gold-erd.mmd"))


def test_gold_erd_stamps_the_toolkit_version_and_nothing_time_varying(tmp_path, monkeypatch):
    """A committed diagram is read long after the run that drew it, so it records which
    build produced it -- and *only* that.

    The ERDs are tracked and drift-gated: CI regenerates them and fails the PR on any
    difference from the committed bytes. A generation timestamp would be a new value on
    every run, in a fresh process each time, so it would fail that gate
    unconditionally. Two separate emits must produce identical bytes.
    """
    hub = _copy_hub(tmp_path)
    monkeypatch.chdir(hub)
    runner = CliRunner()

    assert runner.invoke(cli, ["emit-gold", "party", "--confirm-emit"]).exit_code == 0
    erd = contract_diagrams_dir(hub) / "party-gold-erd.mmd"
    first = erd.read_bytes()
    assert f"%% Generated by kairos-ontology {__version__} -- do not edit".encode() in first

    assert runner.invoke(cli, ["emit-gold", "party", "--confirm-emit"]).exit_code == 0
    assert erd.read_bytes() == first
...
def test_confirm_emit_announces_success_after_the_write(tmp_path, monkeypatch):
    ...
    # Success is announced after every informational line about the product, and
