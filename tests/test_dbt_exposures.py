...
# The Gold dbt artifacts ship in the medallion dbt package via `compile --emit`, not
# beside the Power BI bundle: `powerbi/<domain>/dbt/` has no `dbt_project.yml`, so it
# could never be installed as a dbt package (issue #665).
_EXPOSURES_PATH = "models/gold/invoice/_invoice__exposures.yml"
...
def _generate_gold(domain: str) -> dict[str, str]:
    """Render *domain*'s Gold dbt artifacts at the paths they occupy in the package."""
...
def _gold_product_report(domain: str) -> dict:
    """The projector's own Gold product report, from the Power BI bundle.

    Still sourced from `generate_gold_artifacts`: the report is a Power BI-side
    artifact, unlike the dbt models, which moved into the medallion package (#665).
    """
    graph, namespace, classes = _load_ontology(domain)
    peers = [EXTENSIONS_DIR / "client-silver-ext.ttl"] if domain == "invoice" else []
...
def test_exposures_yaml_depends_on_real_gold_models() -> None:
    ...
    gold_report = _gold_product_report("invoice")
    ...
    emitted_models = {
        ...
        if path.startswith("models/gold/") and path.endswith(".sql")
    }
