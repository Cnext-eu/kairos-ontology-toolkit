...
def _gold_dbt(domain: str, *, gold_path: Path | None = None) -> dict[str, str]:
    ...
    under `<domain>/dbt/` inside the Power BI bundle. That directory has no
    `dbt_project.yml`, so nothing could install it and the copies could only be
    hand-copied downstream (issue #665). They now ship in the medallion dbt package via
    `compile --emit`; this renders the same artifacts directly so the SQL/YAML
    assertions below keep their coverage.
    """
    graph, namespace, classes = _load_ontology(domain)
    peers = [EXTENSIONS_DIR / "client-silver-ext.ttl"] if domain == "invoice" else []
...
def test_dimension_exposure_modes_are_physical(
    ...
    artifacts = _generate("client", gold_path=gold_path)
    gold_dbt = _gold_dbt("client", gold_path=gold_path)
    sql = gold_dbt["models/gold/client/dim_client.sql"]
    assert ("where is_current = 1" in sql) is current_filter
    assert ("models/gold/client/dim_client_current.sql" in gold_dbt) is current_view
...
def test_calendar_is_only_generated_when_explicit_and_approved(
    ...
    assert "models/gold/shared/dim_date.sql" in _gold_dbt("invoice")
    assert (
...
def test_ddl_tmdl_dax_erd_and_report_are_deterministic(invoice_gold):
    ...
    schema = yaml.safe_load(_gold_dbt("invoice")["models/gold/invoice/_invoice__gold_models.yml"])
