...
def test_new_repo_publish_gitignore_tracks_the_dbt_package_and_architecture(tmp_path):
    """new-repo's .gitignore tracks two lanes and ignores every other one.

    The dbt package stays tracked because a dataplatform pins it by SHA and
    subdirectory. `architecture/**` is tracked because the canonical class diagrams and
    DDD maps are how the model is reviewed, and pr-validate.yml regenerates and diffs
    them (#774) -- tracking without that would be worse than not tracking, since the
    files would look authoritative and never be checked. Power BI dropped off the
    allowlist: it is rendered and zipped in CI by ``package-powerbi-release``, so nothing
    needs it committed.
    """
    runner = CliRunner()
    with mock.patch("kairos_ontology.cli.main.subprocess.run") as mock_run:
...
def test_the_drift_gate_regenerates_every_tracked_publish_lane(tmp_path):
    """Anything the template allowlists must be regenerated by the gate that diffs it.

    #774: `architecture/**` comes from `project`, not `compile --emit`, so tracking it
    without adding the projection calls would leave a tree that looks authoritative and
    is never checked. This pins the two halves together.
    """
    ...
    tracked = {
        ...
        for line in gitignore.splitlines()
        if line.strip().startswith("!ontology-hub-publish/") and line.strip().endswith("/**")
    }
